...
-- Insert Synthetic Loans (~1500, approx 1-2 per customer)
-- Use fixed ROWCOUNT=2 and filter to approximate variability
...
FROM BANKING.CUSTOMERS
CROSS JOIN TABLE(GENERATOR(ROWCOUNT => 2))
WHERE ROW_NUMBER() OVER (PARTITION BY CUSTOMER_ID ORDER BY SEQ8()) <= UNIFORM(1, 2, RANDOM());  -- Filter to 1-2 randomly

-- Insert Synthetic Credit Cards (~1500, approx 1-2 per customer)
-- Use fixed ROWCOUNT=2 and filter to approximate variability
